...
void memoized_SW(char* X, char* Y, char** P, int** H, int n, int m){
	//Reference taken from rod cutting example from lecture slides.
	for (int i = 0; i <=n; ++i){ //Initialize 1st column of both H and P to 0
		H[i][0] = 0;
		P[i][0] = '0';		//P has a char data type.
	}
...
int max(int x,int y, int z){			//To get the max value from given 3 values. Basically used for opting out max of p1, p2, p3.
	if(x>=y){
		if(x>=z){
			return x;
		}
...
void print_Seq_Align_X(char* X, char** P, int n, int m){
	...
	if (P[n][m]=='d'){
		...
		cout<<X[n-1];	//Adjusted indexes (n-1) as the first row and columns are initialized with different values
	}
	else{
		if (P[n][m]=='l'){
...
void print_Seq_Align_Y(char* Y, char** P, int n, int m){
	...
	if (P[n][m]=='d'){
		...
		cout<<Y[m-1];	//Adjusted indexes (m-1) as the first row and columns are initialized with different values
	}
	else{
		if (P[n][m]=='l'){
